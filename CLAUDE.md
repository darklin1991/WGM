...
專案骨架、板子設定檔載入、夜晚結算引擎、夜晚流程 UI、完整白天流程（警長競選 → 警上發言 → 公布死訊 → 發言順序 → 逐人計時發言／騎士決鬥 → 放逐投票 → 警徽流）與勝負判定已完成，一整局可以從第一夜跑到分出勝負。已驗證：`flutter analyze` 無問題、435 項測試全數通過、debug APK 建置成功。

內建 6 個板子（見 `assets/presets/`），支援 12 種角色。**各板子的組成、特有規則與修改歷程見 `.claude/板子/`**（一個板子一份，檔名對應 presetId） —— 這裡只寫通則，板子個別的差異寫在那份。
