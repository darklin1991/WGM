...
專案骨架、板子設定檔載入、夜晚結算引擎與夜晚流程 UI 已完成。已驗證：`flutter analyze` 無問題、237 項測試全數通過、debug APK 建置成功。

內建 6 個板子（見 `assets/presets/`），支援 12 種角色。**各板子的組成、特有規則與修改歷程見 `.claude/板子/`**（一個板子一份，檔名對應 presetId） —— 這裡只寫通則，板子個別的差異寫在那份。

尚未實作：白天其餘流程（發言計時、投票放逐、騎士決鬥）、警徽流、勝負判定引擎、復盤日誌與本地持久化。
...
只有定義，夜晚結算不會產生它。決鬥的時機與規則見「騎士決鬥的時機」一節。
...
階段：**上警 → 退水 → 投票 →（平票時 PK 重投）**。警上發言由法官口頭主持，App 不計時也不排順序。
...
⚠️ **警徽流尚未實作** —— 警長死亡時的移交方向還沒做，要等死亡連鎖處理一起。

### 騎士決鬥的時機

騎士只能在**警長競選結束之後、放逐投票開始之前**翻牌決鬥 —— 也就是白天的發言階段。上警階段不可決鬥，投票開始後也不可。

決鬥結果：

| 對手身分 | 結果 | 控制旗標 |
|---|---|---|
| 狼人 | 對手死亡，**立即進入黑夜**（跳過當天投票） | `knightDuelEndsDay` |
| 好人 | 騎士自己出局，白天**照常繼續**走到投票 | — |

決鬥致死的死因是 `DeathCause.knightDuel`。若決鬥掉的是狼美人，被魅惑者**不殉情**（`knightDuelBlocksCharmSuicide`）—— 這是騎士唯一能救下殉情者的時機，夜晚結算碰不到這條。

實作時把「可否決鬥」做成白天狀態機的一個明確視窗，不要散在各處判斷；決鬥出狼要走和自爆相同的「立即進夜」路徑。
...
投票開始前，先確認騎士有沒有要決鬥 —— 決鬥視窗到投票開始就關閉了（見「騎士決鬥的時機」）。
