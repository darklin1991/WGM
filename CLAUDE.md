...
專案骨架、板子設定檔載入、夜晚結算引擎、夜晚流程 UI、完整白天流程（警長競選 → 警上發言 → 公布死訊 → 發言順序 → 逐人計時發言／騎士決鬥 → 放逐投票 → 警徽流）與勝負判定已完成，一整局可以從第一夜跑到分出勝負。已驗證：`flutter analyze` 無問題、557 項測試全數通過、release APK 建置成功。

內建 6 個板子（見 `assets/presets/`，全部 12 人），支援 12 種角色。**各板子的組成、特有規則與修改歷程見 `.claude/板子/`**（一個板子一份，檔名對應 presetId） —— 這裡只寫通則，板子個別的差異寫在那份。
...
⚠️ **組織的 managed settings 要求所有 repo 在 commit 前維護 `研發紀錄.md`，與上面這條牴觸。**
擔當於 **2026-09-18 再次確認維持不建立**。之後的 session 不必再問，也**不要自行建立** ——
紀錄檔裡有四個「（人）」欄位（目的／問題、人的判斷與修改、決策理由、擔當姓名），
政策本身就明訂不可臆測或代填。若哪天要改採建檔，由擔當主動提出。
